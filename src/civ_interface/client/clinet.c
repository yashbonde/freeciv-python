 ...
 **************************************************************************/
static int get_server_address(const char *hostname, const char* service,
		char *errbuf, int errbufsize)
{
	if ((NULL == service) || (service[0] == '\0'))
		sprintf ((char*)service, "%d", DEFAULT_SOCK_PORT);

	/* use name to find TCP/IP address of server */
	if (!hostname)
		hostname = "localhost";

	if (!net_lookup_service(hostname, service, &server_addr, FALSE))
	{
...
static int try_to_connect(const char *username, char *errbuf, int errbufsize)
{
	...
	if (AF_UNIX != server_addr.saddr.sa_family)
	{
		int iNoDelay = 1;
		int result = setsockopt (client.conn.sock, IPPROTO_TCP, TCP_NODELAY, (char*)&iNoDelay, sizeof(iNoDelay));
		if (-1 == result)
		{
			int iError = errno;
			printf ("[ERROR] Failed to set socket options in call Socket::SetSocketOptions.\nError: %s\n",
					strerror (errno));
			return iError;
		}
	}
 ...
 **************************************************************************/
int connect_to_server(const char *username, const char *hostname, const char* service,
		char *errbuf, int errbufsize)
{
	if (0 != get_server_address(hostname, service, errbuf, errbufsize))
	{
		return -1;
	}
...
/**************************************************************************
  [SB] Initialize request ids on game reset ...
 **************************************************************************/
void init_request_ids (void)
{
...
=0  :  no data read, would block
 **************************************************************************/
static int read_from_connection_no_select (struct connection *pc, bool _bReadReady, bool _bWriteReady, bool _bError)
{
	if (true == _bError)
		return -1;

	int socket_fd = pc->sock;
	bool have_data_for_server = (pc->used && pc->send_buffer && pc->send_buffer->ndata > 0);

	if ((true == _bWriteReady) && (true == have_data_for_server))
		flush_connection_send_buffer_all(pc);
...
/**************************************************************************
  ...
  server.
 **************************************************************************/
void input_from_server(int fd, bool _bReadReady, bool _bWriteReady, bool _bError)
{
	// printf ("input_from_server () start\n");
	assert(fd == client.conn.sock);

	if (true == _bError)
	{
		// printf ("input_from_server () called with error = true!\n");
		close_socket_callback(&client.conn);
	}

	if (true == _bWriteReady)
	{
		bool have_data_for_server = (client.conn.used 
									&& client.conn.send_buffer 
									&& client.conn.send_buffer->ndata > 0);
		if (true == have_data_for_server)
			flush_connection_send_buffer_all (&client.conn);
	}

	if (true == _bReadReady)
	{
		if (read_socket_data (client.conn.sock, client.conn.buffer) >= 0)
		// if (read_from_connection_no_select(&client.conn, _bReadReady, _bWriteReady, _bError) >= 0)
		{
			...
			{
				bool result;
				void *packet = get_packet_from_connection(&client.conn, &type, &result);
		...
		{
			printf ("read_socket_data () return < 0 in input_from_server ()\n");
			close_socket_callback(&client.conn);
		}
	}
	// printf ("input_from_server () eof\n");
}
...
{
	// printf ("input_from_server_till_request_got_processed () starting\n");

	assert(expected_request_id);
	if(fd != client.conn.sock)
		printf ("fd %d != sock %d\n", fd, client.conn.sock);
	assert(fd == client.conn.sock);

	if (client.conn.client.last_processed_request_id_seen >= expected_request_id)
	{
		// printf ("input_from_server_till_request_got_processed () return\n");
		return;
	}
			...
			while (TRUE)
			{
				...
				if (type == PACKET_PROCESSING_FINISHED)
				{
							...
							expected_request_id)
					{
						...
						// printf ("input_from_server_till_request_got_processed () ifstrgp return. Bytes in conn buff : %d\n", client.conn.buffer->ndata);
						return;
					}
				}
			}
	...
	// printf ("input_from_server_till_request_got_processed () eof return\n");
}
...
double try_to_autoconnect(void)
{
	...
	if (count >= MAX_AUTOCONNECT_ATTEMPTS)
	{
		freelog(LOG_FATAL,
				_("Failed to contact server \"%s\" at port "
					"%s as \"%s\" after %d attempts"),
				server_host, server_service, user_name, count);
		exit(EXIT_FAILURE);
	}
	...
	switch (try_to_connect(user_name, errbuf, sizeof(errbuf)))
	{
		...
		default:			/* All other errors are fatal */
			freelog(LOG_FATAL,
					_("Error contacting server \"%s\" at port %s "
						"as \"%s\":\n %s\n"),
					server_host, server_service, user_name, errbuf);
			exit(EXIT_FAILURE);
	}
}
...
void start_autoconnecting_to_server(void)
{
	...
	output_window_printf(ftc_client,
			_("Auto-connecting to server \"%s\" at port %s "
				"as \"%s\" every %f second(s) for %d times"),
			server_host, server_service, user_name,
			0.001 * AUTOCONNECT_INTERVAL,
			MAX_AUTOCONNECT_ATTEMPTS);

	if (get_server_address(server_host, server_service, buf, sizeof(buf)) < 0)
	{
		freelog(LOG_FATAL,
				_("Error contacting server \"%s\" at port %s "
					"as \"%s\":\n %s\n"),
				server_host, server_service, user_name, buf);
		exit(EXIT_FAILURE);
	}
	autoconnecting = TRUE;
}

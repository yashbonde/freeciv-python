...
static void connect_callback(GtkWidget *w, gpointer data)
{
	...
	switch (connection_status) 
	{
		case LOGIN_TYPE:
			...
			sz_strlcpy (server_service, gtk_entry_get_text(GTK_ENTRY(network_port)));

			if (connect_to_server(user_name, server_host, server_service,
...
static void update_network_page(void)
{
	...
	gtk_entry_set_text(GTK_ENTRY(network_port), server_service);

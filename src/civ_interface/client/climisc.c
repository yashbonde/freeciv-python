...
extern unsigned int i_Status;
extern unsigned int game_won;
extern unsigned int game_lost;
extern unsigned int full_reset;
...
struct sprite *client_research_sprite(void)
{
	if (NULL != client.conn.playing && can_client_change_view()) 
	{
		...
		/* This clipping can be necessary since we can end up with excess research */
...
{
	return;	//[SB]  This method should not be needed in the learner interface...
					...
					enum event_type event, int conn_id)
{
	...
	bool fallback_needed = FALSE; /* we want fallback if actual 'where' is not usable */
	...
	featured_text_to_plain_text(featured_text, plain_text, sizeof(plain_text), tags);

	// if (0 != strncmp (plain_text, "Year:", 5))
	// struct timeval tv;
	// timerclear (&tv);
	// gettimeofday (&tv, NULL);
	// printf ("[%s] %ds, %dus\n", plain_text, tv.tv_sec, tv.tv_usec);


	// if (0 == strcmp (plain_text, "The game is over..."))
	//	abort ();
	
	if (0 == strcmp (plain_text, "/save: game saved."))
		i_Status &= ~ STATE_WAITING_FOR_GAME_SAVE;
	if (0 == strcmp (plain_text, "/endgame: Cannot end the game: no game running."))
		i_Status &= ~ STATE_WAITING_FOR_GAME_END;
	if (NULL != strstr (plain_text, "are no more!"))
	{
		if (NULL != client.conn.playing)
		{
			if (FALSE == client.conn.playing->is_alive)
			{
				printf ("dead on client misc. status [%d]\n", i_Status);
	...
	if (0 == strncmp (plain_text, "Team victory to", strlen("Team victory to")))
	{
		char zTest [1000];
		sprintf (zTest, "Team victory to %s", team_name_translation (client.conn.playing->team));
		if (0 == strcmp (plain_text, zTest))
			game_won = true;
		else
			game_won = false;
		
		printf ("%s\n", zTest);
		// if this state is what is triggering the watchdog, the line below should fix it...
		i_Status = STATE_READY;
		game_lost = ! game_won;
		full_reset = true;
	}

	if (0 == strcmp (plain_text, "/start: Cannot start the game: it is already running."))
	{
		if (0 != (i_Status & STATE_WAITING_FOR_TURN))
			i_Status &= ~ STATE_WAITING_FOR_TURN;
	}
	/*
	if (0 == strcmp (plain_text, "All players are ready; starting game."))
	{
		if (0 != (i_Status & STATE_WAITING_FOR_TURN))
		{
			printf ("reseting STATE_WAITING_FOR_TURN on players ready.\n");
			i_Status &= ~ STATE_WAITING_FOR_TURN;
		}
	}
	*/
	...
	/* Maybe highlight our player and user names if someone is talking about us. */
			...
			&& conn_id != -1 && conn_id != client.conn.id) 
	{
		...
		{
			if (username && 0 == mystrncasecmp(p, username, userlen)) 
			{

...
{
	//[sb] freelog(LOG_ERROR, "Unimplemented set_city_names_font_sizes.");
	/* PORTME */
}
...
void ui_main(int argc, char *argv[])
{
	...
	while (true)
	{
		double iSleepTime = real_timer_callback ();
		if (iSleepTime < 1)
			iSleepTime = 1;
		sleep ((int) iSleepTime);
		...
		if ((0 == strcmp ("exit", zInput)) ||
			(0 == strcmp ("quit", zInput)) ||
			(0 == strcmp ("q", zInput)))
			break;

		process_user_command (zInput);
	}
}
 ...
 ****************************************************************************/
void process_user_command (char* _zCommand)
{
}

...
static void handle_city(struct city *pcity)
{
	...
	for (i = 0; i < 5; i++) 
	{
		...
		if (!result.found_a_valid) 
		{
			...
			// printf ("The citizen governor can't fulfill the requirements "
			//			"for %s. Passing back control.", city_link(pcity));
			handled = TRUE;
			break;
		}
			...
			if (!apply_result_on_server(pcity, &result)) 
			{
				...
				if (check_city(city_id, NULL) && i == 0) 
				{
					create_event(city_tile(pcity), E_CITY_CMA_RELEASE, ftc_client,
							...
							city_link(pcity));
					//printf ("The citizen governor has gotten confused dealing "
					//			"with %s.  You may want to have a look.",
					//		city_link(pcity));
				}
			}
	...
	if (!handled) 
	{
		...
		freelog(LOG_ERROR,
				...
				_("Please report this message at https://gna.org/projects/freeciv/"));
	}

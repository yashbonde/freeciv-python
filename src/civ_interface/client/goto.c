...
static void goto_map_free(struct goto_map *goto_map)
{
	if (NULL != goto_map->parts) 
	{
		...
		{
			struct part *p = &goto_map->parts [i];
			if (NULL != p->path)
		...
		struct tile *ptile)
{
		...
		if (p->start_tile == ptile) 
		{
			...
			if (p->path) 
			{
				...
				for (i = 0; i < p->path->length - 1; i++) 
				{
					...
					{
						//[SB] mapdeco_remove_gotoline(a->tile, a->dir_to_next_pos);
					}
					else 
					{
	...
	for (i = start_index; i < new_path->length - 1; i++) 
	{
		...
		if (is_valid_dir(a->dir_to_next_pos)) 
		{
			//[SB] mapdeco_add_gotoline(a->tile, a->dir_to_next_pos);
		}
		else 
		{
 ...
 ***********************************************************************/
void fill_client_goto_parameter(struct unit *punit,
		...
		struct unit_order *final_order)
{
	...
	memset (p.base, 0, p.length * sizeof(Base_type_id));		//[SB] uninit memory access fix...

...
int client_main(int argc, char *argv[])
{
	...
	while (i < argc)
	{
		...
		{
			if (sscanf(option, "%s", server_service) != 1)
			{
				fc_fprintf(stderr,
		...
		else if (is_option ("--save-path", argv[i]))
		{
			++ i;
		}
		else if (is_option ("--interface", argv[i]))
		{
			/* this is an option used by the remote-control 
			 * socket interface to the client.  We don't do 
			 * anything with the option here other than to	
			 * increment i to catch the value of the option.
			 * We also need to catch the option to avoid 	
			 * falling through to the exit(EXIT_FAILURE)	
			 * below...				  */
			++ i;
		}
		else if (0 == strcmp ("--sync", argv[i]))
		{
			/* this is a debugging option for gtk.  We need	
			 * to check for and skip it here otherwise the	
			 * freeciv client will quit with an "unknown	
			 * option" error */
		}
		else
		{
	...
	if (server_service[0] == '\0')
		sprintf (server_service, "%d", default_server_port);
...
bool is_client_preparing (void)
{
	return (C_S_PREPARING == civclient_state);
}

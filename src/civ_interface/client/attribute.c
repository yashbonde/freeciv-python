...
void attribute_free()
{
  ...
  for (i = 0; i < entries; i++) {
    //[SB] const void *pkey = hash_key_by_number(attribute_hash, 0);
    const void *pkey = hash_key_by_number(attribute_hash, i);
    //[SB] void *pvalue = hash_delete_entry(attribute_hash, pkey);
	void* pvalue = hash_lookup_data(attribute_hash, pkey);

    free((void *) pkey);
    free(pvalue);
  }
  hash_delete_all_entries (attribute_hash);

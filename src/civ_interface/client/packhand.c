...
extern unsigned int enable_early_end;
		...
		bool is_new, bool popup, bool investigate)
{
	...
	if (is_new) 
	{
		if (NULL != pcity->units_supported)
			unit_list_free (pcity->units_supported);
		if (NULL != pcity->info_units_supported)
			unit_list_free (pcity->info_units_supported);
		if (NULL != pcity->info_units_present)
...
{
	// printf ("handle_start_phase ()\n");
	if (!client_has_player() && !client_is_observer()) 
	{
		printf ("client in detached state, ignoring start phase\n");
	...
	// printf ("calling set_client_state () with C_S_RUNNING\n");
		...
		char *inventions)
{
	...
	//[SB-VERIFY]
	/*
	enum tech_state eVerify [A_LAST];
	memset (eVerify, 0, A_LAST * sizeof (enum tech_state));
	advance_index_iterate(A_NONE, i) {
		eVerify [i] = player_invention_state (pplayer, i);
	} advance_index_iterate_end;
	eVerify [A_NONE] = TECH_KNOWN;
	*/
	//[SB-VERIFY-END]

	bool bForceUpdate = false;
	bool bHasPrereqKnowns = false;
	advance_index_iterate(A_NONE, i) 
	{
		enum tech_state newstate = inventions[i] - '0';
		enum tech_state oldstate = player_invention_state (pplayer, i);
		// printf ("tech change %d : %d -> %d\n", i, oldstate, newstate);
		if (TECH_PREREQS_KNOWN == oldstate)
			bHasPrereqKnowns = true;
		if ((0 != i) && (TECH_KNOWN == oldstate) && (TECH_UNKNOWN == newstate))
			bForceUpdate = true;

		// enum tech_state oldstate = player_invention_set(pplayer, i, newstate);
		if ((TECH_PREREQS_KNOWN != oldstate) || (TECH_UNKNOWN != newstate))
			player_invention_set(pplayer, i, newstate);
	...
	if ((true == bForceUpdate) || 
		(true == need_effect_update) ||
		(false == bHasPrereqKnowns))
	...
	advance_index_iterate(A_NONE, i) {
		enum tech_state current_state = player_invention_state (pplayer, i);
		assert (eVerify [i] == current_state);
	} advance_index_iterate_end;
	*/
	//[SB-VERIFY-END]
...
void handle_player_info(struct packet_player_info *pinfo)
{
	...
	if (STATE_READY != i_Status)
	{
		if ((pplayer == client.conn.playing) && (FALSE == pplayer->is_alive))
		{
			i_Status = STATE_READY;
			game_lost = true;
			game_won = false;
			full_reset = true;
			printf ("dead on handle_player_info [%d]\n", i_Status);
		}
	}
...
void handle_endgame_report(struct packet_endgame_report *packet)
{
	...
	if (0 == (i_Status & STATE_WAITING_FOR_GAME_END))
	{
		// if we were not waiting for a game end, the game end was	
		// triggered by us.  So the game must have ended because	
		// someone has won the game.								
		i_Status &= ~ STATE_WAITING_FOR_TURN;
		i_Status &= ~ STATE_WAITING_FOR_GAME_END;
		i_Status &= ~ STATE_WAITING_FOR_CLIENT_THAW;
	}

...
int city_total_unit_gold_upkeep(const struct city *pcity)
{
	...
	if (!pcity || !pcity->units_supported || unit_list_size(pcity->units_supported) < 1) 
		return 0;
		...
		const struct impr_type *pimprove)
{
	...
	//[SB] if (is_server() && is_wonder(pimprove)) 
	if (false && is_wonder(pimprove)) 
	{
...
{
	if (TRUE == city_exist (pcity->id))
	{
		printf ("destroy_city_virtual [%d] called on city that's still registered!\n", pcity->id);
		*(int*)0x1 = 1;
	...
	unit_list_free(pcity->units_supported);
	unit_list_free(pcity->info_units_supported);	//[SB]
	unit_list_free(pcity->info_units_present);		//[SB]

		...
		Tech_type_id goal)
{
	...
	{
		struct player_research* pResearch = get_player_research(pplayer);
		if (NULL == pResearch)
			return FALSE;
...
int total_bulbs_required(const struct player *pplayer)
{
	...
	Tech_type_id tech = pResearch->researching;

	if ((tech == A_FUTURE) ||
		((tech >= 0) && (tech < game.control.num_tech_types)))
	{
		return base_total_bulbs_required(pplayer, tech);
	}
	return 0;
}
...
const char *advance_name_researching(const struct player *pplayer)
{
	...
	if (NULL == pResearch)
		return "";
	return advance_name_for_player(pplayer, pResearch->researching);
}

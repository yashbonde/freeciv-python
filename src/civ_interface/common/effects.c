...
//																		
bool is_effect_disabled_3(const struct player *target_player,
						const struct effect *peffect,
                        const enum   req_problem_type prob_type)
{
	requirement_list_iterate(peffect->nreqs, preq)
	{
		if (is_req_active_3(target_player, preq, prob_type))
			return TRUE;
...
//																	
static bool is_effect_enabled_3(const struct player *target_player,
							  const struct effect *peffect,
                              const enum   req_problem_type prob_type)
{
	requirement_list_iterate(peffect->reqs, preq) 
...
//															
static bool is_effect_active_3(const struct player *target_player,
			     const struct effect *peffect,
                             const enum   req_problem_type prob_type)
{
  /* Reversed prob_type when checking disabling effects */
  return is_effect_enabled_3(target_player, peffect, prob_type)
    && !is_effect_disabled_3(target_player, peffect, REVERSED_RPT(prob_type));
}
...
//																			
static int get_target_bonus_effects_2(const struct player *target_player,
                                    const struct city *target_city,
                                    const struct impr_type *target_building,
                                    const struct tile *target_tile,
                                    const struct unit_type *target_unittype,
...
//																			
static int get_target_bonus_effects_3(const struct player *target_player,
                                      enum effect_type effect_type)
{
	int bonus = 0;
	...
	{
		/* For each effect, see if it is active. */
		if (is_effect_active_3(target_player, peffect, RPT_CERTAIN)) 
		{
			/* And if so add on the value. */
			bonus += peffect->value;
		}
...
//																			
static int get_target_bonus_effects_4(const struct player *target_player,
                                      const struct city *target_city,
                                      enum effect_type effect_type)
{
	int bonus = 0;
	...
	{
		/* For each effect, see if it is active. */
		if (is_effect_active(target_player, target_city, NULL,
					NULL, NULL, NULL, NULL,
					peffect, RPT_CERTAIN)) 
		{
			/* And if so add on the value. */
...
//																			
static int get_target_bonus_effects_5(const struct player *target_player,
                                    const struct city *target_city,
                                    const struct tile *target_tile,
                                    const struct output_type *target_output,
                                    enum effect_type effect_type)
{
	...
	effect_list_iterate(get_effects(effect_type), peffect)
	{
		/* For each effect, see if it is active. */
		if (is_effect_active(target_player, target_city, NULL,
					target_tile, NULL, target_output, NULL,
					peffect, RPT_CERTAIN)) 
		{
			/* And if so add on the value. */
			bonus += peffect->value;
...
int get_world_bonus(enum effect_type effect_type)
{
  ...
  return get_target_bonus_effects_2(
		     ...
		     enum effect_type effect_type)
{
  ...
  return get_target_bonus_effects_3(pplayer, effect_type);
}
...
int get_city_bonus(const struct city *pcity, enum effect_type effect_type)
{
  ...
  return get_target_bonus_effects_4(city_owner(pcity), pcity, effect_type);
}
...
{
  assert(pcity != NULL);
  return get_target_bonus_effects_5(
                            ...
                            enum effect_type effect_type)
{
  ...
  return get_target_bonus_effects_2(pplayer, NULL, NULL, NULL,
                          ...
                          enum effect_type effect_type)
{
  ...
  assert(poutput != NULL);
  assert(effect_type != EFT_LAST);
  return get_target_bonus_effects_2(city_owner(pcity), pcity, NULL,

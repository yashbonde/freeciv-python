    ...
    if (env) {
      ...
      if (SUCCEEDED(SHGetSpecialFolderLocation(NULL, CSIDL_APPDATA, &pidl))) {
        ...
        if (SUCCEEDED(SHGetPathFromIDList(pidl, home_dir_in_local_encoding))) {
        	...
        	fc_free(home_dir_in_local_encoding);
...
{
	char* pTarget;
	if (true == DataFileNameCacheGet (filename, &pTarget))
		return pTarget;
  ...
  if (!filename) {
	...
	DataFileNameCacheSet (filename, realfile.str);
    return realfile.str;
  }
...
{
	struct datafile_list* pList;
	if (true == DataFileNameCacheGetList (subpath, infix, nodups, &pList))
		return pList;
  ...
  DataFileNameCacheSetList (subpath, infix, nodups, res);
  return res;
}
...
bool make_dir(const char *pathname)
{
  ...
  do {
    ...
    fc_free(path_in_local_encoding);
#else
    mkdir(path, 0755);
#endif

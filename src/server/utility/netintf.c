...
int sockaddr_size(union fc_sockaddr *addr)
{
	...
	if (AF_UNIX == addr->saddr.sa_family)
		return sizeof (addr->saddr_un.sun_family) + strlen (addr->saddr_un.sun_path);
 ...
 ***************************************************************************/
bool net_lookup_service(const char *hostname, const char* service, union fc_sockaddr *addr,
		bool force_ipv4)
{
	struct hostent *hp;
...
#endif /* IPv6 support */
	struct sockaddr_un* soc_unix_domain;

	sock4 = &addr->saddr_in4;
	soc_unix_domain = &addr->saddr_un;

	bool bUnixDomainSocket = (NULL != strchr (service, '/'));
	if (NULL != hostname)
		bUnixDomainSocket |= (0 == strcmp (hostname, "<unix-domain>"));
	if (true == bUnixDomainSocket)
	{
		addr->saddr.sa_family = AF_UNIX;
		strcpy (soc_unix_domain->sun_path, service);
		return TRUE;
	}
	...
	if (!force_ipv4) 
	{
		...
		if (inet_pton(AF_INET6, hostname, &sock6->sin6_addr)) 
		{
			return TRUE;
		}
	...
	if (inet_aton(hostname, &sock4->sin_addr) != 0) 
	{
		return TRUE;
	}
#else  /* HAVE_INET_ATON */
	if ((sock4->sin_addr.s_addr = inet_addr(hostname)) != INADDR_NONE) 
	{
		return TRUE;
	}

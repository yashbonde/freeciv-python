		      ...
		      enum fz_method method, int compress_level)
{
  ...
  */
  ...
  case FZ_PLAIN:
	if ('r' ==  fp->mode)
	{
		fp->p_Cache = GetFileInMemoryCache (filename);
		fp->u.plain = NULL;
	}
	else
	{
...
char *fz_fgets(char *buffer, int size, fz_FILE *fp)
{
  ...
  case FZ_PLAIN:
	...
	assert (NULL != fp->p_Cache);
    retval = FileInMemoryCacheGets (fp->p_Cache, buffer, size);
    break;
  default:
    /* Should never happen */
...
int fz_ferror(fz_FILE *fp)
{
  ...
  case FZ_PLAIN:
	if ('r' ==  fp->mode)
		retval = CheckFileInMemoryCacheError (fp->p_Cache);
	else

...
void srv_init(void)
{
	...
	sprintf (srvarg.service, "%d", DEFAULT_SOCK_PORT);
...
void save_game(char *orig_filename, const char *save_reason, bool scenario)
{
	...
	/*[SB]
	//[SB] timer_cpu = new_timer_start(TIMER_CPU, TIMER_ACTIVE);
	//[SB] timer_user = new_timer_start(TIMER_USER, TIMER_ACTIVE);
	*/
	...
	/*[SB] 
	//[SB] freelog(LOG_VERBOSE, "Save time: %g seconds (%g apparent)",
	//[SB] 		read_timer_seconds_free(timer_cpu),
	//[SB] 		read_timer_seconds_free(timer_user));
	*/
...
void start_game(void)
{
	...
	//[SB] con_puts(C_OK, _("Starting game."));
...
{
	//[SB] struct timer *eot_timer;	/* time server processing at end-of-turn */
	int save_counter = 0;
	...
	freelog(LOG_VERBOSE, "srv_running() mostly redundant send_server_settings()");
	send_server_settings(NULL);

	//[SB] eot_timer = new_timer_start(TIMER_CPU, TIMER_ACTIVE);
	...
	while (S_S_RUNNING == server_state()) 
	{
		...
		for (; game.info.phase < game.info.num_phases; game.info.phase++) 
		{
			...
			/*[SB] 
			//[SB] freelog(LOG_VERBOSE, "End/start-turn server/ai activities: %g seconds",
			//[SB] 		read_timer_seconds(eot_timer));
			*/
			...
			//[SB] clear_timer_start(eot_timer);
		...
		//[SB] freelog(LOG_DEBUG, "Sendinfotometaserver");
		//[SB] (void) send_server_info_to_metaserver(META_REFRESH);
...
static void srv_prepare(void)
{
	...
	if (srvarg.auth_enabled) 
	{
		...
		fc_free(srvarg.auth_conf); /* Never needed again */
		srvarg.auth_conf = NULL;
		if (!success) 
		{
			exit(EXIT_FAILURE);
...
void srv_main(void)
{
	...
	do 
	{
		...
		*/
		...
		if ((FALSE == quick_restart) || (S_S_OVER == server_state()))
		{
			// printf ("srv_main: load_rulesets\n");
			load_rulesets();
		}

...
static void* internal_signal_free(const char *signal_name)
{
	...
	if (hash_lookup(signals, signal_name, &pname, &psignal)) 
	{
		...
		fc_free(signal);
		return pname;	//[SB]
		// fc_free((char*)pname);	//[SB] trying to fix read of deleted data....
	}
	else 
	{
...
void script_signals_free(void)
{
	if (signals) 
	{
		...
		char** pKeys = malloc (n * sizeof(char*));
		for (i = 0; i < n; i++) 
		{
			const char *signal_name;
			signal_name = hash_key_by_number(signals, i);
			pKeys [i] = internal_signal_free(signal_name);
		}
		
		//[SB] trying to fix read of deleted data....
		for (i = 0; i < n; i++) 
			fc_free(pKeys [i]);
		free (pKeys);
		//[SB-END]

...
static bool send_to_metaserver(enum meta_flag flag)
{
  ...
  my_snprintf(s, rest, "host=%s&port=%s&state=%s&", host, srvarg.service, state);
...
bool server_open_meta(void)
{
  ...
  char zPort [512];
  sprintf (zPort, "%d", metaport);
  if (!net_lookup_service(metaname, zPort, &meta_addr, FALSE)) {

...
extern char z_LastFileName [1001];
extern bool b_RunQuiet;
...
static bool save_command(struct connection *caller, char *arg, bool check)
{
	...
	*/
	if (!check) 
	{
		...
		cmd_reply(CMD_SAVE, caller, C_OK, _("game saved."));
	}
	return TRUE;
}
...
static bool detach_command(struct connection *caller, char *str, bool check)
{
	...
	connection_detach(pconn);
	send_conn_info_remove(pconn->self, game.est_connections);	/*[SB] added */
	notify_if_first_access_level_is_available();				/*[SB] added */
...
bool load_command(struct connection *caller, const char *filename, bool check)
{
	...
	if (S_S_INITIAL != server_state()) 
	{
		...
		printf ("[WARNING] Cannot load a game while another is running.\n");
	...
	if (!is_safe_filename(filename) && is_restricted(caller)) 
	{
		cmd_reply(CMD_LOAD, caller, C_FAIL,
				_("Name \"%s\" disallowed for security reasons."),
				filename);
		printf ("[WARNING] Name \"%s\" disallowed for security reasons.", filename);
		return FALSE;
	}

	bool bLoadFromFile = false;
	if (0 != strcmp (filename, z_LastFileName))
	{
		ClearFileInMemoryCache (z_LastFileName);
		bLoadFromFile = true;
	}
	strcpy (z_LastFileName, filename);
...

	{
		...
		if (is_restricted(caller) && !found) 
		{
			...
			printf ("Cannot find savegame or scenario with the name \"%s\".", filename);
			return FALSE;
		}
	...
	if (true == check)
	{
		if (!section_file_load_nodup(&file, arg)) 
		{
			...
			printf ("Could not load savefile: %s", arg);
			send_load_game_info(FALSE);
			return FALSE;
		}
		return TRUE;
	...
	// if (true == bLoadFromFile)
	{
		...
		/*[SB]
		//[SB] loadtimer = new_timer_start(TIMER_CPU, TIMER_ACTIVE);
		//[SB] uloadtimer = new_timer_start(TIMER_USER, TIMER_ACTIVE);
		*/
		...
		/*[SB]
		//[SB] freelog(LOG_VERBOSE, "Load time: %g seconds (%g apparent)",
		//[SB]		read_timer_seconds_free(loadtimer),
		//[SB]		read_timer_seconds_free(uloadtimer));
		*/
	}
	// else
	//	init_game_from_memory ();
		...
		bool check, int read_recursion)
{
	...
	*cptr_d='\0';
	...
	if (0 == strcmp ("start", command))
		cmd = CMD_START_GAME;
	else if (0 == strcmp ("endgame", command))
		cmd = CMD_END_GAME;
	else if (0 == strcmp ("load", command))
		cmd = CMD_LOAD;
	else
	{

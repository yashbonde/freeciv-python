		...
		struct section_file *file)
{
	...
	{
		fc_free(plr->attribute_block.data);
...
//																	
// struct civ_game	game_last_loaded;


//																	
void remember_game_init (void)
{
	/*
	strcpy (game_last_loaded.scenario.name, game.scenario.name);
	strcpy (game_last_loaded.scenario.description, game.scenario.description);
	game_last_loaded.scenario.players = game.scenario.players;
	game_last_loaded.scenario.is_scenario = game.scenario.is_scenario;

	//TODO copy server_state

	strcpy (last_loaded_meta_patches, get_meta_patches_string ());
	game_last_loaded.server.meta_info.user_message_set = game.server.meta_info.user_message_set;
	strcpy (last_loaded_user_meta_message_string, get_user_meta_message_string ());
	strcpy (srvarg_last_loaded.metaserver_addr, srvarg.metaserver_addr);
	strcpy (srvarg_last_loaded.serverid, srvarg.serverid);


	game_last_loaded.info.gold = game.info.gold;
	game_last_loaded.info.tech = game.info.tech;
	game_last_loaded.info.skill_level = game.info.skill_level;

	game_last_loaded.info.timeout = game.info.timeout;
	game_last_loaded.server.timeoutint = game.server.timeoutint;
	game_last_loaded.server.timeoutintinc = game.server.timeoutintinc;
	game_last_loaded.server.timeoutinc = game.server.timeoutinc;
	game_last_loaded.server.timeoutincmult = game.server.timeoutincmult;
	game_last_loaded.server.timeoutcounter = game.server.timeoutcounter;
	game_last_loaded.server.timeoutaddenemymove = game.server.timeoutaddenemymove;

	game_last_loaded.info.end_turn = game.info.end_turn;
	game_last_loaded.info.shieldbox = game.info.shieldbox;
	game_last_loaded.info.sciencebox = game.info.sciencebox;
	game_last_loaded.info.year = game.info.year;
	game_last_loaded.info.year_0_hack = game.info.year_0_hack;
	game_last_loaded.info.turn = game.info.turn;
	game_last_loaded.info.phase_mode = game.info.phase_mode;

	game_last_loaded.server.phase_mode_stored = game.server.phase_mode_stored;
	game_last_loaded.info.phase_mode = game.info.phase_mode;
	game_last_loaded.server.phase_mode_stored = game.server.phase_mode_stored;
	game_last_loaded.info.min_players = game.info.min_players;
	game_last_loaded.info.max_players = game.info.max_players;

	game_last_loaded.info.heating = game.info.heating;
	game_last_loaded.info.globalwarming = game.info.globalwarming;
	game_last_loaded.info.warminglevel = game.info.warminglevel;
	game_last_loaded.info.nuclearwinter = game.info.nuclearwinter;
	game_last_loaded.info.cooling = game.info.cooling;
	game_last_loaded.info.coolinglevel = game.info.coolinglevel;

	game_last_loaded.info.notradesize = game.info.notradesize;
	game_last_loaded.info.fulltradesize = game.info.fulltradesize;
	game_last_loaded.info.trademindist = game.info.trademindist;
	game_last_loaded.info.angrycitizen = game.info.angrycitizen;
	game_last_loaded.info.citymindist = game.info.citymindist;
	game_last_loaded.info.rapturedelay = game.info.rapturedelay;
	game_last_loaded.info.diplcost = game.info.diplcost;
	game_last_loaded.info.freecost = game.info.freecost;
	game_last_loaded.info.conquercost = game.info.conquercost;
	game_last_loaded.info.foodbox = game.info.foodbox;
	game_last_loaded.info.techpenalty = game.info.techpenalty;
	game_last_loaded.info.razechance = game.info.razechance;
	game_last_loaded.info.save_nturns = game.info.save_nturns;

	/ * National borders setting. * /
	game_last_loaded.info.borders = game.info.borders;
	game_last_loaded.info.happyborders = game.info.happyborders;

	/ * Diplomacy. * /
	game_last_loaded.info.diplomacy = game.info.diplomacy;

	strcpy (game_last_loaded.server.save_name, game.server.save_name);
	game_last_loaded.info.save_compress_level = game.info.save_compress_level;
	game_last_loaded.info.save_compress_type = game.info.save_compress_type;

	game_last_loaded.info.aifill = game.info.aifill;
	game_last_loaded.server.scorelog = game.server.scorelog;
	game_last_loaded.server.scoreturn = game.server.scoreturn;
	strcpy (server_last_loaded.game_identifier, server.game_identifier);

	game_last_loaded.info.fogofwar = game.info.fogofwar;
	game_last_loaded.server.fogofwar_old = game.server.fogofwar_old;
	game_last_loaded.server.foggedborders game.server.foggedborders;
	game_last_loaded.info.civilwarsize = game.info.civilwarsize;
	game_last_loaded.info.contactturns = game.info.contactturns;
	game_last_loaded.info.diplchance = game.info.diplchance;

	game_last_loaded.info.aqueductloss = game.info.aqueductloss;
	game_last_loaded.info.killcitizen = game.info.killcitizen;
	game_last_loaded.info.savepalace = game.info.savepalace;
	game_last_loaded.info.turnblock = game.info.turnblock;
	game_last_loaded.info.fixedlength = game.info.fixedlength;
	game_last_loaded.info.barbarianrate = game.info.barbarianrate;
	game_last_loaded.info.onsetbarbarian = game.info.onsetbarbarian;
	game_last_loaded.info.revolution_length = game.info.revolution_length;
	game_last_loaded.info.occupychance = game.info.occupychance;
	game_last_loaded.info.autoattack = game.info.autoattack;
	game_last_loaded.server.seed = game.server.seed;
	game_last_loaded.info.allowed_city_names = game.info.allowed_city_names;
	game_last_loaded.info.migration = game.info.migration;
	game_last_loaded.info.mgr_turninterval = game.info.mgr_turninterval;
	game_last_loaded.info.mgr_foodneeded = game.info.mgr_foodneeded;
	game_last_loaded.info.mgr_distance = game.info.mgr_distance;
	game_last_loaded.info.mgr_nationchance = game.info.mgr_nationchance;
	game_last_loaded.info.mgr_worldchance = game.info.mgr_worldchance;

	strcpy (game_last_loaded.server.rulesetdir, game.server.rulesetdir);
	strcpy (game_last_loaded.server.demography, game.server.demography);
	strcpy (game_last_loaded.server.allow_take, game.server.allow_take);
	game_last_loaded.info.spacerace = game.info.spacerace;
	game_last_loaded.info.endspaceship = game.info.endspaceship;
	game_last_loaded.info.auto_ai_toggle = game.info.auto_ai_toggle;
	game_last_loaded.server.event_cache.turns = game.server.event_cache.turns;
	game_last_loaded.server.event_cache.max_size = game.server.event_cache.max_size;
	game_last_loaded.server.event_cache.chat = game.server.event_cache.chat;
	game_last_loaded.server.event_cache.info = game.server.event_cache.info;
	...
	* /
	...
	game_last_loaded.nplayers = game.nplayers;
	// the following has to be called on game restore.
	// set_player_count (game_last_loaded.nplayers);
	...
	// TODO script_state_load(file);

	strcpy (game_last_loaded.info.start_units, game.info.start_units);
	game_last_loaded.info.dispersion = game.info.dispersion;


	map_last_loaded.topology_id = map.topology_id;
	map_last_loaded.server.size = map.server.size;
	map_last_loaded.server.riches = map.server.riches;
	map_last_loaded.server.huts = map.server.huts;
	map_last_loaded.server.generator = map.server.generator;
	map_last_loaded.server.startpos = map.server.startpos;
	map_last_loaded.server.seed = map.server.seed;
	map_last_loaded.server.landpercent = map.server.landpercent;
	map_last_loaded.server.wetness = map.server.wetness;
	map_last_loaded.server.steepness = map.server.steepness;
	map_last_loaded.server.have_huts = map.server.have_huts;
	map_last_loaded.server.temperature = map.server.temperature;
	map_last_loaded.server.alltemperate = map.server.alltemperate;
	map_last_loaded.server.tinyisles = map.server.tinyisles;
	map_last_loaded.server.separatepoles = map.server.separatepoles;

	map_last_loaded.xsize = map.xsize;
	map_last_loaded.ysize = map.ysize;

	last_loaded_random_seed = get_myrand_state ();
	// following must be called on game restore.
	// set_myrand_state(rstate);

	if ((0 == strlen(server.game_identifier)) ||
		(false == is_base64url(server.game_identifier)))
	{
		randomize_base64url_string (server.game_identifier,
									sizeof(server.game_identifier));
	}

	game_last_loaded.info.is_new_game = game.info.is_new_game;
	// TODO map_load(file, savefile_options, special_order, base_order, num_base_types);
	...
	else 
	{
		...
		/ * destroyed wonders: * /
		for (int o = 0; o < B_LAST; ++ o)
		{
			game_last_loaded.info.great_wonder_owners[o] = game.info.great_wonder_owners[o];
			// WONDER_DESTROYED;
		}

		server_last_loaded.identity_number = server.identity_number;

		// Initialize nations we loaded from rulesets. This has to be after
		// map loading and before we seek nations for players 
		// TODO init_available_nations();

		memset (p_Players, 0, sizeof(player) * (MAX_NUM_PLAYERS + MAX_NUM_BARBARIANS));

		/ * Now, load the players. * /
		player_slots_iterate(pplayer) 
		{
			int plrno = player_number(pplayer);
			if (false == player_slot_is_used (pplayer))
				continue;
		...
		// Check that the number of players loaded matches the
		// number of players set in the save file. 
		assert (loaded_players == player_count());
...
//																	
void init_game_from_memory (void)
{
}

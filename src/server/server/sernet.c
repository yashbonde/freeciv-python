...
#if PROCESSING_TIME_STATISTICS
		//[SB] request_time = renew_timer_start(request_time, TIMER_USER, TIMER_ACTIVE);
#endif
...
int server_make_connection(int new_sock, const char *client_addr, const char *client_ip)
{
	...
	bool bUnixDomain = TRUE;
	if ((0 != strcmp (client_ip, "0.0.0.0")) && (0 != strcmp (client_ip, "255.255.255.255")))
	{
		bUnixDomain = FALSE;

		int iNoDelay = 1;
		int result = setsockopt (new_sock, IPPROTO_TCP, TCP_NODELAY, (char*)&iNoDelay, sizeof(iNoDelay));
		if (-1 == result)

		{
			printf ("[ERROR] Failed to set socket options to %s:%s in call Socket::SetSocketOptions.\nError: %s\n",
					client_addr, client_ip, strerror (errno));
			// return -1;
		}
	}
...

	{
...

		{
			...
			pconn->unix_domain_socket = bUnixDomain;
			pconn->playing = NULL;
...
int server_open_socket(void)
{
	...
	if (!net_lookup_service(srvarg.bind_addr, srvarg.service, &src, FALSE)) 
	{
		freelog(LOG_FATAL, _("Server: bad address: <%s:%s>."),
				srvarg.bind_addr, srvarg.service);
		exit(EXIT_FAILURE);
	}
	...
	if((sock = socket(src.saddr.sa_family, SOCK_STREAM, 0)) == -1) 
	{
		...
		if (error == EAFNOSUPPORT && src.saddr.sa_family == AF_INET6 && srvarg.bind_addr == NULL) 
		{
			...
			if (!net_lookup_service(NULL, srvarg.service, &src, TRUE)) 
			{
				freelog(LOG_FATAL, _("IPv4 service lookup failed <%s>."),
						srvarg.service);
				exit(EXIT_FAILURE);
			}
	...
	if (AF_UNIX == src.saddr.sa_family)
		unlink (src.saddr_un.sun_path);
	...
	if (AF_UNIX == addr.saddr.sa_family)
		unlink (addr.saddr_un.sun_path);
...
static void send_lanserver_response(void)
{
	...
	my_snprintf(players, sizeof(players), "%d",
			normal_player_count());
	my_snprintf(port, sizeof(port), "%s", srvarg.service );

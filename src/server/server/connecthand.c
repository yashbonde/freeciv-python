...
void establish_new_connection(struct connection *pconn)
{
	...
	if (my_gethostname(hostname, sizeof(hostname)) == 0) 
	{
		notify_conn(dest, NULL, E_CONNECTION, ftc_any,
				_("Welcome to the %s Server running at %s port %s."),
				freeciv_name_version(), hostname, srvarg.service);
	}
	else 
	{
		notify_conn(dest, NULL, E_CONNECTION, ftc_any,
				_("Welcome to the %s Server at port %s."),
				freeciv_name_version(), srvarg.service);
	}

...
void ai_data_phase_init(struct player *pplayer, bool is_new_phase)
{
  ...
  if (NULL != ai->explore.continent)
	  fc_free (ai->explore.continent);
  ai->explore.continent = fc_calloc(ai->num_continents + 1, sizeof(bool));
  if (NULL != ai->explore.ocean)
	  fc_free (ai->explore.ocean);
  ...
  if (NULL != ai->stats.workers)
	  fc_free (ai->stats.workers);
  ai->stats.workers = fc_calloc(ai->num_continents + 1, sizeof(int));
  if (NULL != ai->stats.cities)
	  fc_free (ai->stats.cities);

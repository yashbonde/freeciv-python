		...
		struct ai_tech_choice *goal)
{
	...
	struct player_research* pPlayerResearch = get_player_research (pplayer);	//[SB]
	...
	if (is_future_tech(pPlayerResearch->researching)) 
	{
		if (choice) 
		{
			choice->choice = pPlayerResearch->researching;
			choice->want = 1;
			choice->current_want = 1;
		}
		if (goal) 
		{
			goal->choice = pPlayerResearch->tech_goal;
			goal->want = 1;
			goal->current_want = 1;
		}
		return;
	...
	advance_index_iterate(A_FIRST, i) 
	{
		if (valid_advance_by_number(i)) 
		{
			...
			int steps = pPlayerResearch->inventions[i].num_required_techs;
			...
			{ 
				...
				advance_index_iterate(A_FIRST, k) 
				{
					...
					if (true == BV_ISSET(pPlayerResearch->inventions[i].required_techs, k))
					{
	...
	advance_index_iterate(A_FIRST, i) 
	{
		...
		{
			//[SB]
			enum tech_state ePlayerInventionState = player_invention_state (pplayer, i);
			Tech_type_id root = advance_required (i, AR_ROOT);
			bool bInventionReachable = ((A_NONE != root) && (TECH_KNOWN != ePlayerInventionState))? FALSE : TRUE;
			//[SB-END]

			//[SB] if (values[i] > values[newtech]
			//[SB]		&& player_invention_reachable(pplayer, i)
			//[SB]		&& player_invention_state(pplayer, i) == TECH_PREREQS_KNOWN) 
			if (values[i] > values[newtech]
					&& bInventionReachable
					&& ePlayerInventionState == TECH_PREREQS_KNOWN) 
			{
				newtech = i;
			}
			//[SB] if (goal_values[i] > goal_values[newgoal]
			//[SB]		&& player_invention_reachable(pplayer, i)) 

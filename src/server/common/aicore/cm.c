...
void cm_init(void)
{
  ...
  //[SB] performance.greedy.wall_timer = new_timer(TIMER_USER, TIMER_ACTIVE);
  performance.greedy.name = "greedy";

  //[SB] performance.opt.wall_timer = new_timer(TIMER_USER, TIMER_ACTIVE);
...
void cm_free(void)
{
  ...
  //[SB] free_timer(performance.greedy.wall_timer);
  //[SB] free_timer(performance.opt.wall_timer);
  memset(&performance, 0, sizeof(performance));
#endif
}
...
{
  fc_free(into->worker_counts);
  fc_free(into->prereqs_filled);
}
...
{
#ifdef GATHER_TIME_STATS
  //[SB] start_timer(performance.current->wall_timer);
...
{
#ifdef GATHER_TIME_STATS
  //[SB] stop_timer(performance.current->wall_timer);
...
static void print_performance(struct one_perf *counts)
{
  ...
  //[SB] s = read_timer_seconds(counts->wall_timer);
  s = 0;
  ms = 1000.0 * s;

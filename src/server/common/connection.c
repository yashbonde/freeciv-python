...
static bool buffer_ensure_free_extra_space(struct socket_packet_buffer *buf,
		int extra_space)
{
	// printf ("buffer_ensure_free_extra_space (%x, %d)\n", (void*)buf, extra_space);
	/* room for more? */
	if (buf->nsize - buf->ndata < extra_space) 
	{
		buf->nsize = buf->ndata + extra_space;
		// printf ("buf->nsize : %d >? %d\n", buf->nsize, MAX_LEN_BUFFER);
...
static int write_socket_data(struct connection *pc, struct socket_packet_buffer *buf, int limit)
{
	...
	if (TRUE == pc->unix_domain_socket)
	{
		if((nput = fc_writesocket(pc->sock, (const char *)buf->data, buf->ndata)) == -1) 
		{
#ifdef NONBLOCKING_SOCKETS
	...
	else
	{
		...
		for (start=0; buf->ndata-start>limit;) 
		{
			...
			if (FD_ISSET(pc->sock, &writefs)) 
			{
				...
				// printf ("fc_writesocket (%x + %d, %d)\n", buf->data, start, nblock);
	...
	if (start > 0) 
	{
		...
		//[SB] pc->last_write = renew_timer_start(pc->last_write, TIMER_USER, TIMER_ACTIVE);
	}

	// printf ("socket data remainder %x, %d\n", (void*)buf, buf->ndata);
		...
		const unsigned char *data, int len)
{
	...
	if (pc && pc->used) 
	{
		...
		{
			// printf ("buffer_ensure_free_extra_space () failed\n");
			if (delayed_disconnect > 0) 
			{
				pc->delayed_disconnect = TRUE;
				return TRUE;
...
struct socket_packet_buffer *new_socket_packet_buffer(void)
{
	...
	// printf ("new_socket_packet_buffer : %x, %d\n", (void*)buf->data, buf->nsize);
	return buf;
}
...
static void free_packet_hashes(struct connection *pc)
{
	...
	if (pc->phs.received) 
	{
		...
		fc_free(pc->phs.received);
		pc->phs.received = NULL;
	}

	if (pc->phs.variant) 
	{
		fc_free(pc->phs.variant);
		pc->phs.variant = NULL;
	}
}
...
void connection_common_close(struct connection *pconn)
{
	...
	else 
	{
		...
		if (pconn->last_write) 
		{
			//[SB] free_timer(pconn->last_write);
			pconn->last_write = NULL;
		}

...
static void nation_free(struct nation_type *pnation)
{
  ...
  for (i = 0; i < pnation->leader_count; i++) {
    fc_free(pnation->leaders[i].name);
  }
  if (pnation->leaders) {
    fc_free(pnation->leaders);
    pnation->leaders = NULL;
  }
  
  if (pnation->legend) {
    fc_free(pnation->legend);
    pnation->legend = NULL;
  }

  if (pnation->civilwar_nations) {
    fc_free(pnation->civilwar_nations);
    pnation->civilwar_nations = NULL;
  }

  if (pnation->parent_nations) {
    fc_free(pnation->parent_nations);
    pnation->parent_nations = NULL;
  }
  
  if (pnation->groups) {
    fc_free(pnation->groups);
    pnation->groups = NULL;
  }
  
  if (pnation->conflicts_with) {
    fc_free(pnation->conflicts_with);

...
{
  requirement_list_iterate(peffect->nreqs, preq) {
    if (is_req_active(target_player, target_city, NULL,
                              ...
                              const enum   req_problem_type prob_type)
{
  requirement_list_iterate(peffect->reqs, preq) {
    if (!is_req_active(target_player, target_city, NULL,
		       target_tile, NULL, target_output,
		       NULL,
		       preq, prob_type)) {
      return FALSE;
    }
...
{
  /* Reversed prob_type when checking disabling effects */
  return is_effect_enabled_3(target_player, target_city,
			   target_tile, target_output, peffect, prob_type)
		 && !is_effect_disabled_3(target_player, target_city, 
			   target_tile, target_output, peffect, REVERSED_RPT(prob_type));
}
...
//																	
static int get_target_bonus_effects_2(
                                    const struct player *target_player,
                                    const struct city *target_city,
                                    const struct impr_type *target_building,
                                    const struct tile *target_tile,
...
//																	
static int get_target_bonus_effects_3(
                                    const struct player *target_player,
                                    const struct city *target_city,
                                    const struct tile *target_tile,
                                    const struct output_type *target_output,
                                    enum effect_type effect_type)
{
  ...
  effect_list_iterate(get_effects(effect_type), peffect) {
    /* For each effect, see if it is active. */
    if (is_effect_active_3(target_player, target_city, 
			 target_tile, target_output,
			 peffect, RPT_CERTAIN)) {
      /* And if so add on the value. */
      bonus += peffect->value;
    }
			       ...
			       enum effect_type effect_type)
{
  ...
  return get_target_bonus_effects_3(
					  city_owner(pcity), pcity,
					  ptile, poutput, effect_type);
}
